...
import SingleCard from "./ProductCard";
    ...
    <div className="grid items-center container-main mb-5 ">
      <div className="grid  grid-cols-1 sm:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-16 my-10">
        {productCard.map((item, idx) => (
          <SingleCard id={String(idx)} img={item.image} name={item.name}  star={item.rating } isLiked={true} price={item?.price as number} color={item.color} isNew={item.isNew} discountPrice={Number(item.isDiscount)  } />
        ))}
      </div>
